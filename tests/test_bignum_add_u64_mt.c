...
void* thread_func(void* arg) {
    ...
    for (int i = 0; i < NUM_ITERATIONS; ++i) {
        ...
        bignum_add_u64_status_t status = bignum_add_u64(&res, &data->a, data->b);

        if (status != BIGNUM_ADD_U64_SUCCESS || !bignum_are_equal(&res, &data->expected)) {
            data->ok = 0;
            break;
        }
    }
...
int main(void) {
    printf("\n--- Starting MT test for bignum_add_u64 ---\n");
    ...
    for (int i = 0; i < NUM_THREADS; ++i) {
        ...
        bignum_add_u64_status_t st = bignum_add_u64(&data[i].expected, &data[i].a, data[i].b);
        if (st != BIGNUM_ADD_U64_SUCCESS) {
    ...
    if (!all_ok) {
        fprintf(stderr, "--- MT test for bignum_add_u64 FAILED ---\n");
        return 1;
    }

    printf("\n--- MT test for bignum_add_u64 passed ---\n");
    return 0;
}
