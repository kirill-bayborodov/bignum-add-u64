/**
 * @file    bench_bignum_add_u64.c
 * @brief   Микробенчмарк для профилирования bignum_add_u64.
 * @author  git@bayborodov.com
 * @version 1.0.0
 * @date    29.07.2026
 *
 * @details
 *   Вызывает функцию bignum_add_u64 на случайных
 ...
 *    benchmarks/bench_bignum_add_u64.c build/bignum_add_u64.o \
 *    -o bin/bench_bignum_add_u64
 *
 * # Запуск perf с записью стека через frame-pointer
 * /usr/local/bin/perf record -F 9999 -o benchmarks/reports/report_bench_bignum_add_u64 -g -- bin/bench_bignum_add_u64
 */
...
static void init_random_bignum(bignum_t *num) {
    ...
    // ошибок переполнения (OVERFLOW) при сложении,
...
int main(void) {
    ...
    for (uint32_t i = 0; i < ITERATIONS; ++i) {
        ...
        bignum_add_u64(&res_dst, &a[data_idx], b[data_idx]);
